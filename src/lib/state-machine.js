 ...
 * 容错：idle 且 chanceSpent=true 也允许 reveal——多标签页下，他页刷新会把
 * 持久化的 scratching 归一为 idle，本页刮满结算时不应因此失败（次数已扣、
 * 奖品已锁定，reveal 只推进展示态，不重摇）。
 */
export function transitionReveal(state, { cardId }) {
  ...
  const canReveal =
    ...
    (card.state === CARD_STATE.IDLE && card.chanceSpent)
  if (!canReveal) {
    return { state, result: { ok: false, reason: 'invalid-state' } }
  }
  const next = cloneState(state)
...
export function normalizeAfterReload(state) {
  ...
  for (const card of Object.values(next.cards)) {
    if (card.state === CARD_STATE.SCRATCHING) card.state = CARD_STATE.IDLE
  }
  return next
}
 ...
 * chancesUsed 以 dailyChances 为上界钳制——这是"落盘次数永不超上限"的兜底
 * 护栏，即使无锁环境下发生丢失更新，存储值也不可能越过每日上限。
 */
export function sanitizeDailyState(input, { cardIds, date, dailyChances }) {
  ...
  base.chancesUsed = Number.isFinite(chancesUsed)
    ? Math.min(cap, Math.max(0, Math.floor(chancesUsed)))
    : 0
