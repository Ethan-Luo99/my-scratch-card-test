...
export function createSyncChannel({ name, backend, channelFactory = defaultChannel, senderId }) {
  ...
  if (!bc && backend && backend.subscribe) {
    ...
    unsubscribeBackend = backend.subscribe((key) => {
      if (key === name) dispatch({ name, rev: null })
    })
  }
