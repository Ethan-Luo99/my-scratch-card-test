 ...
 */
export function migrateData(parsed, { date, cardIds, maxChances, createDailyStateFn, sanitizeFn }) {
  ...
  if (version === STORAGE_VERSION && parsed.state && typeof parsed.state === 'object') {
    ...
    const state = sanitizeFn(parsed.state, { cardIds, date, maxChances })
  ...
  state.chancesUsed = Number.isFinite(used)
    ? Math.min(limit, Math.max(0, Math.floor(used)))
    : 0
